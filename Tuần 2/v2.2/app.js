...
// Lưu cả transactions lẫn budgets
function save() {
...
function renderBudget() {
  ...
  if (cats.length === 0) {
    el.innerHTML = '<div class="no-budget">Chưa có hạn mức nào. Thêm hạn mức bên dưới!</div>';
    return;
  }
  ...
  const html = cats.map(cat => {
    ...
    const pct = Math.min((spentAmt / limit) * 100, 100); // max 100% cho thanh bar
...
function renderList(filtered) {
  ...
  if (filtered.length === 0) {
    el.innerHTML = '<div class="empty">Không có giao dịch nào phù hợp bộ lọc.</div>';
    return;
  }
...
function render() {
  ...
  renderBudget();           // Cập nhật thanh hạn mức  ← MỚI ở v2.1
  renderAlerts();           // Cập nhật cảnh báo       ← MỚI ở v2.1
}

// ===========================
// Dữ liệu mẫu
function addSample() {
  // Lấy năm và tháng hiện tại để dữ liệu mẫu khớp với cảnh báo
  const now = new Date();
  ...
  budgets['Giải trí'] = budgets['Giải trí'] || 200000;
  budgets['Học tập'] = budgets['Học tập'] || 400000;
  budgets['Đi lại'] = budgets['Đi lại'] || 150000;
...
// Nhấn Enter ở ô số tiền giao dịch để thêm nhanh
document.getElementById('tx-amount').addEventListener('keydown', function (e) {
  if (e.key === 'Enter') addTransaction();
});

// Nhấn Enter ở ô hạn mức để thêm nhanh
...
load();   // Tải dữ liệu từ localStorage
